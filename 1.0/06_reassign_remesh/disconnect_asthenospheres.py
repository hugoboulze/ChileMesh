...
Disconnect oceanic and continental asthenospheres
...
m = zset.Mesh('./mesh_asthenospheres_to_disconnect.geo')

to_remove = ~m.elsets['ASTHENO_TO_DISCONNECT'].mask() # ~ means inverse of the table
...
mm = zset.Mesh('./mesh_asthenospheres_to_disconnect.geo')

if mean_coord0 > mean_coord1: #oceanic part is the one with the lower mean y
	mm.elsets.add("ASTHENO_OCEAN_tmp", ids=ids1)
else:
	mm.elsets.add("ASTHENO_OCEAN_tmp", ids=ids0)

mm.save('./mesh_asthenospheres_disconnected.geo')
